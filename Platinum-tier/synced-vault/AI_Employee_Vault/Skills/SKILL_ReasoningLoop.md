...
```
Use reasoning-loop: Run Platinum Tier cross-zone coordination loop. Monitor synced-vault/AI_Employee_Vault/cloud-zone/ and local-zone/ folders. Apply claim-by-move rule. Process cloud drafts, coordinate local approvals, execute zone-specific actions. Sync between zones. Continue until all tasks complete.
```
